...
当前仓库自动化环境不执行真实 Gmail 客户端截图，因此以最终邮件 HTML 契约作为可重复的视觉替代证据：回归测试要求“标的 / 动作 / 主预测 / 量化方向 / 机会 / 风险”六个表头分别存在于独立 `<th>` 单元格中，同时验证 `<table>` 带有邮件端稳定布局声明。真实邮件客户端的最终观感仍应在下一次生产统一日报发送后人工抽查。

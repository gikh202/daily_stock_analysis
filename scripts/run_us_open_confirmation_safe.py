...
def _apply_wait_better_entry_contract(payload: dict[str,Any]) -> dict[str,Any]:
    """Make the WAIT research contract explicit without introducing future data.

    The reference price is generated upstream by the V7.1 intraday timing model from
    signal-time inputs only. This function deliberately does not inspect market data;
    it only validates that reference and persists the settlement metadata needed by
    the research ledger.
    """
    decisions=payload.get("decisions")
    if not isinstance(decisions,list): return payload
    for decision in decisions:
        if not isinstance(decision,dict) or str(decision.get("action") or "").upper()!="WAIT_BETTER_ENTRY":
            continue
        price=_finite(decision.get("current_price")); target=_finite(decision.get("expected_better_price"))
        if price is None or target is None or target<=0 or target>=price:
            symbol=str(decision.get("symbol") or "UNKNOWN")
            raise RuntimeError(f"{symbol} WAIT_BETTER_ENTRY missing valid expected_better_price")
        decision["expected_wait_minutes"]=30
        decision["better_entry_reason"]="atr_pullback"
    return payload


def _finalize_wait_better_entry_contract(result: dict[str,Any]) -> dict[str,Any]:
    json_path=Path(str(result.get("json") or ""))
    if not json_path.is_file():
        raise RuntimeError("V7 timing output JSON missing before WAIT contract finalization")
    payload=json.loads(json_path.read_text(encoding="utf-8"))
    if not isinstance(payload,dict):
        raise RuntimeError("V7 timing output JSON must be an object")
    _apply_wait_better_entry_contract(payload)
...
def _run_once(*,v6_payload_path: str|Path,output_dir: str|Path,source_run_id: str|None,notify: bool,previous_state_path: str|Path|None,force_notify: bool,allow_all_unavailable: bool) -> dict[str,Any]:
    result=run_timing(v6_payload_path=v6_payload_path,output_dir=output_dir,source_run_id=source_run_id,notify=notify,previous_state_path=previous_state_path,force_notify=force_notify,allow_all_unavailable=allow_all_unavailable,**_policy())
    return _finalize_wait_better_entry_contract(result)

...
def _llm_health(
    ...
    if (
        model_text
        and primary_model
        and model_text.lower() != primary_model.strip().lower()
    ):
...
def _market_breadth(context: Dict[str, Any]) -> Optional[str]:
    ...
    if isinstance(breadth, dict):
        return str(breadth.get("breadth") or "").strip().lower() or None
    return str(regime.get("breadth") or "").strip().lower() or None


def _qualitative_intelligence(
    raw_result: Dict[str, Any],
    ...
    return (
        _as_text_tuple(intelligence.get("positive_catalysts") if intelligence else None),
        _as_text_tuple(intelligence.get("risk_alerts") if intelligence else None),
    )
...
def _effective_trade_date(
    ...
    value = _find_value(
        context,
        ("effective_daily_bar_date", "effective_trade_date", "last_complete_trade_date"),
    )
    text = str(value or "").strip()
    if len(text) >= 10:
        return text[:10]
        ...
        *,
        ...
        context_features = {
            ...
            "forecast_intelligence": {
                ...
                "challenger_model": bundle.challenger_model,
        ...
        return V6Signal(
            ...
            opportunity_score=alpha.opportunity_score,
            risk_score=alpha.risk_score,
            evidence_coverage=float(bundle.evidence_coverage),
            market_regime=adapted.market_regime,
            market_breadth=_market_breadth(context),
            ...
            diagnostics={
                ...
                "forecast_v7": bundle.to_dict(),
                "forecast_decision": forecast_decision.to_dict(),

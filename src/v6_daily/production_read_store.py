...
from .production_common import (
    V6_PRODUCTION_SCOREBOARD_SCHEMA_VERSION,
    calibration,
    finite,
    return_hit_rate,
    spearman,
...
PRODUCTION_READ_SCHEMA_VERSION = "v6-production-normalized-read-v1"
...
class ProductionV6ReadStore:
    """Stage 11 read-only business adapter backed only by normalized facts."""
    ...
    def daily_deltas(self) -> List[Dict[str, Any]]:
        ...
        for item in self.latest_board():
            ...
            def delta(field: str) -> Optional[float]:
                now = finite(item.get(field))
                before = finite(previous.get(field))
            ...
            horizons.append(
                {
                    ...
                    "buy_setup_hit_rate_pct": return_hit_rate(buy_rows, positive=True),
                    "avoidance_samples": len(avoid_rows),
                    "avoidance_hit_rate_pct": return_hit_rate(avoid_rows, positive=False),
                    ...
                    "calibration": calibration(bucket, minimum),
                }
            )

        return {
            "schema_version": V6_PRODUCTION_SCOREBOARD_SCHEMA_VERSION,

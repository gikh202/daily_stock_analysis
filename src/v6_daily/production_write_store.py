...
from .normalized_schema import ensure_normalized_schema
from .production_common import finite, horizon_days, json_compact, parse_date, utc_now


PRODUCTION_WRITE_SCHEMA_VERSION = "v6-production-normalized-write-v1"
PRODUCTION_WRITE_MODE = "normalized_only_no_legacy_projection"
...
class ProductionV6WriteStore:
    """Stage 11 production writer backed only by registered normalized tables.

    This module intentionally has no import or SQL dependency on legacy V6
    stores, legacy persistence, compatibility projection, or legacy identity.
    """
    ...
    def __init__(self, path: str, *, active_engine_version: str) -> None:
        ...
        self.schema_registry = ensure_normalized_schema(self.path)
        self.signal_identity_migrated = False
        self.bootstrap_summary: Dict[str, Any] = {
            "performed": False,
            "automatic": False,
            "reason": "legacy_bootstrap_is_explicit_cli_only",
        }
    ...
    def has_signal_key(self, code: str, effective_trade_date: Any, engine_version: str) -> bool:
        effective = parse_date(effective_trade_date)
        with self.connect() as conn:
            row = conn.execute(
                """
                SELECT 1
    ...
    def save_signal(self, signal: V6Signal, *, engine_version: str) -> bool:
        ...
        if engine != self.active_engine_version:
            raise ValueError(
                "ProductionV6WriteStore only accepts the active engine version: "
                f"{engine!r} != {self.active_engine_version!r}"
            )
        effective = signal.effective_trade_date or parse_date(signal.analysis_created_at)
        ...
        with self.connect() as conn:
            ...
            conn.execute(
                ...
                (
                    ...
                    json_compact(signal.features),
                    json_compact(signal.context_features),
                    json_compact(diagnostics),
                ),
            )
            ...
            conn.execute(
                ...
                (
                    ...
                    finite(signal.quality_score),
                    finite(signal.opportunity_score),
                    finite(signal.risk_score),
                    float(signal.evidence_coverage),
                    packet.execution.status.value,
                    int(bool(packet.execution.actionable)),
                    json_compact(packet_dict),
                ),
            )
            ...
            conn.execute(
                ...
                (
                    ...
                    json_compact(list(packet.execution.targets)),
                    float(packet.execution.max_position_pct),
                    packet.execution.risk_reward,
                    json_compact(list(packet.execution.confirmations)),
                    json_compact(list(packet.execution.invalidations)),
                    int(bool(packet.execution.has_active_plan)),
                    int(bool(packet.execution.actionable)),
                    json_compact(execution_dict),
                ),
            )
            conn.commit()
    ...
    def all_signals(self) -> List[Dict[str, Any]]:
        ...
        for row in rows:
            ...
            item["horizon_forecasts_json"] = json_compact(
        ...
        with self.connect() as conn:
            ...
            conn.execute(
                ...
                (
                    ...
                    hit,
                    finite(forecast_score),
                    normalized_direction,
                    finite(benchmark_spy_return_pct),
                    finite(benchmark_qqq_return_pct),
    ...
    def write_status(self) -> Dict[str, Any]:
        ...
        return {
            "schema_version": PRODUCTION_WRITE_SCHEMA_VERSION,
            "mode": PRODUCTION_WRITE_MODE,
            ...
            "schema_registry": dict(self.schema_registry),

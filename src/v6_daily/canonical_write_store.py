...
from .versioned_store import VersionedV6DailyStore


CANONICAL_WRITE_SCHEMA_VERSION = "v6-normalized-write-primary-v1"
CANONICAL_WRITE_MODE = "normalized_primary_legacy_projection"
...
class CanonicalV6WriteStore(VersionedV6DailyStore):
    """Write normalized V6 domain facts first, then project legacy compatibility rows.

    The normalized tables are the canonical write destination for new LIVE V6
    signals and forecast outcomes. ``v6_signals`` / ``v6_outcomes`` remain only
    as compatibility projections while Accuracy Lab and older consumers finish
    migrating. Both sides are written in one SQLite transaction, with the
    normalized insert executed first; any projection/parity failure rolls back
    the whole fact.

    Existing pre-cutover databases are bootstrapped once from legacy rows before
    new canonical writes are accepted. That bootstrap is migration-only and does
    not change historical business values.
    """

    def __init__(self, path: str, *, active_engine_version: str) -> None:
        super().__init__(path, active_engine_version=active_engine_version)
        self._normalized = NormalizedV6Persistence(str(self.path))
        ...
        self._write_counters: Dict[str, int] = {
            ...
            "legacy_signal_projections": 0,
            "canonical_outcomes_inserted": 0,
            "legacy_outcome_projections": 0,
        }
        self._bootstrap_existing_legacy_rows()

    def _bootstrap_existing_legacy_rows(self) -> None:
        ...
        payload = {
            "version": self.active_engine_version,
            "generated_at": _utc_now(),
            "board": super().latest_board(),
        }
        summary = self._normalized.persist_snapshot(
            payload,
        ...
        self.bootstrap_summary = {
            ...
            "forecast_outcomes": summary.get("forecast_outcomes"),
    ...
    def _next_identity(
        ...
        *,
        ...
        legacy_max = int(
            conn.execute(f"SELECT COALESCE(MAX(id), 0) FROM {legacy_table}").fetchone()[0]
        )
        normalized_max = int(
            conn.execute(
        ...
        return max(legacy_max, normalized_max) + 1
    ...
    def save_signal(self, signal: V6Signal, *, engine_version: str) -> bool:
        ...
        if engine != self.active_engine_version:
            raise ValueError(
                "CanonicalV6WriteStore only accepts the active engine version: "
        ...
        with self.connect() as conn:
            ...
            source_signal_id = self._next_identity(
                conn,
                legacy_table="v6_signals",
            ...
            # Legacy is now a compatibility projection. Keep the same integer
            # identity so old consumers and parity checks can compare rows exactly.
            key = self.signal_key(signal.code, effective, engine)
            conn.execute(
                """
                ...
                (
                    ...
                    effective,
                    key,
                    _json(signal.horizon_forecasts),
                    _json(signal.context_features),
                ),
            )

            projected = conn.execute(
                "SELECT id, engine_version FROM v6_signals WHERE id=?",
                (source_signal_id,),
            ).fetchone()
            if projected is None or str(projected["engine_version"]) != engine:
                raise RuntimeError("legacy signal projection parity failed")
            conn.commit()

        self._write_counters["canonical_signals_inserted"] += 1
        self._write_counters["legacy_signal_projections"] += 1
        return True
    ...
    def all_signals(self) -> List[Dict[str, Any]]:
        ...
        for row in rows:
            ...
            item["horizon_forecasts_json"] = _json(horizons if isinstance(horizons, dict) else {})
            result.append(item)
        return result
    ...
    def save_outcome(
        ...
        *,
        ...
        rounded_return = round(return_pct, 6)
        rounded_mfe = None if mfe is None else round(mfe, 6)
        rounded_mae = None if mae is None else round(mae, 6)
        rounded_spy = None if excess_spy is None else round(excess_spy, 6)
        rounded_qqq = None if excess_qqq is None else round(excess_qqq, 6)
        ...
        with self.connect() as conn:
            ...
            source_outcome_id = self._next_identity(
                conn,
                legacy_table="v6_outcomes",
            ...
            conn.execute(
                ...
                INSERT INTO v6_outcomes(
                    id, signal_id, horizon_days, evaluated_at, end_trade_date,
                    start_price, end_price, return_pct, mfe_pct, mae_pct,
                    directional_hit, forecast_score, direction_used,
                    benchmark_spy_return_pct, benchmark_qqq_return_pct,
                    excess_vs_spy_pct, excess_vs_qqq_pct
            ...
            projected = conn.execute(
                "SELECT id, signal_id, horizon_days FROM v6_outcomes WHERE id=?",
                (source_outcome_id,),
            ).fetchone()
            if (
                projected is None
                or int(projected["signal_id"]) != int(signal_id)
                or int(projected["horizon_days"]) != int(horizon_days)
            ):
                raise RuntimeError("legacy outcome projection parity failed")
            conn.commit()

        self._write_counters["canonical_outcomes_inserted"] += 1
        self._write_counters["legacy_outcome_projections"] += 1
        return True
    ...
    def write_status(self) -> Dict[str, Any]:
        with self.connect() as conn:
            ...
            missing_legacy_signals = int(
                conn.execute(
                    """
                    SELECT COUNT(*)
                    FROM v6_forecast_runs f
                    LEFT JOIN v6_signals s
                      ON s.id=f.source_signal_id AND s.engine_version=f.engine_version
                    WHERE f.engine_version=? AND s.id IS NULL
                    """,
                    (self.active_engine_version,),
                ).fetchone()[0]
            )
            missing_canonical_signals = int(
                conn.execute(
                    """
                    SELECT COUNT(*)
                    FROM v6_signals s
                    LEFT JOIN v6_forecast_runs f
                      ON f.source_signal_id=s.id AND f.engine_version=s.engine_version
                    WHERE s.engine_version=? AND f.id IS NULL
                    """,
                    (self.active_engine_version,),
                ).fetchone()[0]
            )
            ...
            missing_legacy_outcomes = int(
                conn.execute(
                    """
                    SELECT COUNT(*)
                    FROM v6_forecast_outcomes no
                    JOIN v6_forecast_runs f ON f.id=no.forecast_run_id
                    LEFT JOIN v6_outcomes lo ON lo.id=no.source_outcome_id
                    WHERE f.engine_version=? AND lo.id IS NULL
                    """,
                    (self.active_engine_version,),
                ).fetchone()[0]
            )
            missing_canonical_outcomes = int(
                conn.execute(
                    """
                    SELECT COUNT(*)
                    FROM v6_outcomes lo
                    JOIN v6_signals s ON s.id=lo.signal_id
                    LEFT JOIN v6_forecast_outcomes no ON no.source_outcome_id=lo.id
                    WHERE s.engine_version=? AND no.id IS NULL
                    """,
                    (self.active_engine_version,),
                ).fetchone()[0]
            )
        ...
        parity_exact = (
            canonical_signals == legacy_signals
            and canonical_outcomes == legacy_outcomes
            and missing_legacy_signals == 0
            and missing_canonical_signals == 0
            and missing_legacy_outcomes == 0
            and missing_canonical_outcomes == 0
            and not fk_errors
            and quick.strip().lower() == "ok"
        )
        return {
            "schema_version": CANONICAL_WRITE_SCHEMA_VERSION,
            "mode": CANONICAL_WRITE_MODE,
            ...
            "legacy_signal_projections": legacy_signals,
            "canonical_outcomes": canonical_outcomes,
            "legacy_outcome_projections": legacy_outcomes,
            "missing_legacy_signal_projections": missing_legacy_signals,
            "missing_canonical_signal_rows": missing_canonical_signals,
            "missing_legacy_outcome_projections": missing_legacy_outcomes,
            "missing_canonical_outcome_rows": missing_canonical_outcomes,

# -*- coding: utf-8 -*-
"""Deterministic trend-evidence cleanup for LLM prompting.

This module isolates prompt-only consistency handling from ``src.analyzer``.
It works on a derived copy of already-computed trend evidence and does not
mutate provider/runtime configuration, fetch market data, alter technical
indicator calculations, score trades, apply execution guardrails, persist
state, notify users, or participate in WAIT_BETTER_ENTRY behavior.
"""
...
def _safe_float(value: Any, default: float = 0.0) -> float:
    """Safely convert to float with the analyzer's historical fallback semantics."""
    if value is None:
        return default
...
def _infer_trend_direction(trend: Dict[str, Any]) -> str:
    """Infer the final prompt trend direction from status and MA alignment."""
    combined = " ".join(
...
def _filter_conflicting_trend_items(
    ...
    """Drop reasons that directly conflict with the inferred trend direction."""
...
def _sanitize_trend_analysis_for_prompt(
    ...
    *,
    ...
    """Clean prompt-only trend hints on a derived copy of the evidence."""

...
# 1. Strict no-lookahead and paired forward-only Champion/Challenger evaluation.
replace(
...
paired_selection = '''    def paired_model_metrics(
        self,
        *,
        as_of_date: str | None,
        horizon_days: int,
...
regex_replace(
    "src/forecasting/history.py",
    r"    def select_champion\(.*\Z",
    paired_selection,
)


# 2. Version bump plus independent Champion/Challenger selection per horizon.
replace(
...
# 3. Execution gates are authoritative rather than diagnostic-only.
replace(
...
# 4. Intraday better-entry metric is explicitly an uncalibrated heuristic score.
replace(
    "src/forecasting/timing.py",
    '    version = "v7.0-intraday-timing.1"',
    '    version = "v7.1-intraday-timing.1"\n    better_entry_metric = "heuristic_score_v1"',
)
replace(
    "src/forecasting/timing.py",
    'f"near-term pullback probability {better:.1%} with estimated {expected_improvement:.2f}% price improvement; expected value favors waiting"',
...
replace(
    "src/forecasting/timing.py",
    'f"waiting edge is not material (better-entry probability {better:.1%}, estimated improvement {expected_improvement:.2f}%); current setup remains executable"',
...
replace(
    ...
    'POLICY_VERSION="us-open-timing-v7.0"',
    'POLICY_VERSION="us-open-timing-v7.1"',
)
replace(
    "scripts/run_us_open_timing.py",
    'forecast_confidence:float|None; better_entry_probability:float;',
    'forecast_confidence:float|None; better_entry_score:float; better_entry_probability:float;',
)
replace(
    "scripts/run_us_open_timing.py",
    'starter_position_pct=0.0,better_entry_probability=0.0,expected_better_price=None',
    'starter_position_pct=0.0,better_entry_score=0.0,better_entry_probability=0.0,expected_better_price=None',
)
replace(
    "scripts/run_us_open_timing.py",
    'starter_position_pct=base.starter_position_pct if timing.action=="BUY_NOW" else 0.0,better_entry_probability=timing.better_entry_probability,expected_better_price=',
    'starter_position_pct=base.starter_position_pct if timing.action=="BUY_NOW" else 0.0,better_entry_score=timing.better_entry_probability,better_entry_probability=timing.better_entry_probability,expected_better_price=',
)
replace(
    "scripts/run_us_open_timing.py",
    "更好买点概率 | 预计更优价",
...
replace(
    "scripts/run_us_open_timing.py",
    '_pct(x.better_entry_probability,probability=True)',
    '_pct(x.better_entry_score,probability=True)',
)
replace(
    "scripts/run_us_open_timing.py",
    '未来近端出现更好买点概率 {_pct(x.better_entry_probability,probability=True)}',
    '更好买点启发式评分（未校准） {_pct(x.better_entry_score,probability=True)}',
)
replace(
    "scripts/run_us_open_timing.py",
    '"- 概率和期望收益是基于历史校准的估计，不是收益保证。",""]',
    '"- 收盘层上涨概率和期望收益来自历史校准，不是收益保证。","- `更好买点评分` 当前是盘中启发式 score，不是校准概率；Research Ledger 会按多时点 outcome 验证，样本不足前不得表述为胜率。",""]',
)

regex_replace(
    "scripts/run_us_open_timing.py",
    r"def _signature\(decisions:Sequence\[OpenTimingDecision\]\)->str:.*?(?=\ndef _read_previous)",
...
regex_replace(
    "scripts/run_us_open_timing.py",
    r"def _should_notify\(previous:Mapping\[str,Any\]\|None,\*,signature:str,generated_at:datetime,force:bool\)->bool:.*?(?=\ndef _notify)",
...
replace(
    "scripts/run_us_open_timing.py",
    'payload={"version":"us-open-timing-v7","policy_version":POLICY_VERSION,',
    'payload={"version":"us-open-timing-v7.1","policy_version":POLICY_VERSION,"better_entry_metric":{"field":"better_entry_score","legacy_alias":"better_entry_probability","semantics":"heuristic_score","calibrated":False},',
)


# 5. Research ledger v2: correct artifact chain, action field and multi-timepoint outcomes.
regex_replace(
    "scripts/us_open_research_ledger.py",
    r"def connect\(path: str \| Path\) -> sqlite3.Connection:.*?(?=\ndef signal_key)",
...
regex_replace(
    "scripts/us_open_research_ledger.py",
    r"def compute_outcome\(row: Mapping\[str, Any\], frame: Any\) -> dict\[str, Any\] \| None:.*?(?=\ndef settle_pending)",
...
regex_replace(
    "scripts/us_open_research_ledger.py",
    r"def summary\(path: str \| Path\) -> dict\[str, Any\]:.*?(?=\ndef export_summary)",
...
replace(
    ...
    "startsWith('us-open-confirmation-')",
    "startsWith('us-open-timing-')",
)
replace(
    ".github/workflows/02b-us-open-research-ledger.yml",
    "pattern: us-open-confirmation-*",
    "pattern: us-open-timing-*",
)


# 6. Regression tests for every audited failure mode.
replace(
...
'''
text = load("tests/test_us_open_research_ledger.py")
if "test_record_signal_keeps_multiple_intraday_observations" not in text:
    save("tests/test_us_open_research_ledger.py", text + ledger_tests)
...
'''
text = load("tests/test_v7_forecast_calibration.py")
if "test_history_rows_reject_missing_or_invalid_source_dates" not in text:
    save("tests/test_v7_forecast_calibration.py", text + forecast_tests)
...
'''
text = load("tests/test_notification_streamlining.py")
if "test_unchanged_timing_state_is_not_resent" not in text:
    save("tests/test_notification_streamlining.py", text + notify_tests)


# 7. Docs and changelog match the corrected contracts.
replace(
...
replace(
    "docs/v7-forecast-architecture.md",
    "其内部生产 Forecast Engine 已升级到 `v7.0-forecast.1`",
    "其内部生产 Forecast Engine 已升级到 `v7.1-forecast.1`",
)
replace(
    "docs/us-open-runtime-confirmation.md",
    "`等更好买点` 会估计近端出现更优入场价格的概率、预计改善幅度和参考更优价；它不是看空，而是表示等待的期望值高于立即追入。",
    "`等更好买点` 会输出近端更优入场的**启发式评分（未校准）**、预计改善幅度和参考更优价；它不是看空，而是表示当前公式认为等待的期望值高于立即追入。Research Ledger 在足够 outcome 形成前不会把这个评分称为概率。",
)
replace(
    "docs/us-open-runtime-confirmation.md",
    "状态发生实质变化或长时间未更新时才重新通知",
    "只有动作、启发式评分分档或相对入场/止损区间发生实质变化时才重新通知",
)
replace(
    "docs/us-open-runtime-confirmation.md",
    "- 预测概率与证据覆盖率是两个独立概念。低历史样本时概率会做先验收缩并标明校准状态，不会把“数据齐全”伪装成“高胜率”。",
    "- 预测概率与证据覆盖率是两个独立概念。5D 仍为 `prior_only`、预测可信度不足、Alpha 非正或分布 R:R 不足时，执行层 fail-closed 为 WAIT/零新仓，不会只记录 gate 后继续授权仓位。\n- Research Ledger 使用 `us-open-timing-*` Artifact，并按分钟 bar 时间保存同一标的一天内的多次评估；次日结算等待是否真的获得参考更优价，为未来盘中评分校准提供真实样本。",
)

changelog = load("docs/CHANGELOG.md")
marker = "## [Unreleased]\n\n"
line = "- [修复] V7.1 修复 post-merge 审计发现的预测与盘中学习漏洞：strict no-lookahead 排除无效历史日期，Champion/Challenger 改为各 horizon 独立且仅用 paired forward-only 样本晋级，所有执行 gate 真正 fail-closed；恢复 `us-open-timing-*` Research Ledger 链路并按分钟多时点记录/结算等待是否获得更优价，将盘中“更好买点概率”降级为明确未校准的启发式评分，并以语义价格区间抑制无实质变化的重复邮件。\n"
if line not in changelog:
    if marker not in changelog:
        raise SystemExit("docs/CHANGELOG.md: Unreleased marker missing")
    save("docs/CHANGELOG.md", changelog.replace(marker, marker + line, 1))

print("V7 post-merge audit patch applied")

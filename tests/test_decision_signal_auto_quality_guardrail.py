...
def test_standalone_builder_without_snapshot_remains_backward_compatible() -> None:
    payload = build_decision_signal_payload_from_report(
        _result(),
        context_snapshot=None,
        trace_id="quality-standalone",
        query_source="system",
        report_type="simple",
        profile_source="auto_default",
    )
...
def test_fresh_auto_buy_fails_closed_when_persisted_snapshot_lacks_quality_contract() -> None:
    payload = build_decision_signal_payload_from_report(
        _result(),
        context_snapshot={},
        trace_id="quality-contract-missing",
        query_source="system",
        report_type="simple",
        profile_source="auto_default",
    )
    ...
    assert payload["action"] == "watch"
    assert payload["metadata"]["data_quality_level"] == "unknown"
    assert payload["metadata"]["data_quality_guardrail_reason"] == "insufficient_data_quality:unknown"

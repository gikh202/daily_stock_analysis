...
def test_production_gate_allows_only_exact_normalized_live_read() -> None:
...
def test_production_gate_treats_manual_legacy_fallback_as_diagnostic_only() -> None:
    ...
    assert gate["legacy_policy"] == "diagnostic_only_not_production_ready"
    ...
    with pytest.raises(RuntimeError, match="production read gate blocked"):
        assert_production_read_gate(payload)


def test_production_gate_blocks_integrity_or_parity_drift() -> None:
    mutations = [
